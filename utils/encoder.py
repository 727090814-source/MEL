...
# h5py 在当前环境中可能与 numpy 存在二进制兼容问题，这里做一个安全导入：
try:
...
class BaseEncoder:
    ...
    def _save_embeddings(self, embeddings: torch.Tensor, ids: List, file_path: str):
        ...
        - 若 h5py 可用，则写入 H5 文件；
        - 否则退化为 torch.save(.pt)，避免二进制兼容问题。
        """
        os.makedirs(os.path.dirname(file_path), exist_ok=True)
        if H5PY_AVAILABLE:
            with h5py.File(file_path, 'w') as f:  # type: ignore
                f.create_dataset('embeddings', data=embeddings.cpu().numpy(), compression='gzip')
                f.create_dataset('ids', data=np.array(ids))
            self.logger.info(f"Saved embeddings to H5 {file_path}")
            print(f"Saved embeddings to H5 {file_path}")
        else:
            torch.save({"embeddings": embeddings.cpu(), "ids": ids}, file_path + ".pt")
            self.logger.info(f"Saved embeddings to PT {file_path}.pt (h5py not available)")
            print(f"Saved embeddings to PT {file_path}.pt (h5py not available)")
